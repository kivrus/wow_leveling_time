...
const xpPerLevel = [
  ...
  // Benchmark XP/hour based on community brackets (from forums / MMO-Champion)
  const segmentRates = {
    lvl1_10: 15425,
    lvl10_20: 14100,
    lvl20_30: 14525,
  ...
  function getSegmentRate(lvl) {
    if (lvl <= 10)  return segmentRates.lvl1_10;
    if (lvl <= 20) return segmentRates.lvl10_20;
    if (lvl <= 30) return segmentRates.lvl20_30;
    if (lvl <= 40) return segmentRates.lvl30_40;
    if (lvl <= 50) return segmentRates.lvl40_50;
    return segmentRates.lvl50_60;
  }
  ...
  function calculate() {
    ...
    for (let i = level - 1; i < xpPerLevel.length; i++) {
      const lvl     = i + 1;
      const segRate = getSegmentRate(lvl);
      const effRate = xpPerHour * (segRate / baseXpPerHour);
  ...
  function drawChart(playerRate, dailyHours, baseRate, startLevel) {
    ...
    for (let i = startLevel - 1; i < xpPerLevel.length; i++) {
      const lvl     = i + 1;
      const segRate = getSegmentRate(lvl);
      const effRate = playerRate * (segRate / baseRate);
    ...
    chart = new Chart(ctx, {
      ...
      options: {
        ...
        scales: {
          x: {
            type: 'linear', position: 'bottom', offset: true,
